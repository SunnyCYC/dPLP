...
fontsize = 12
figsize = (8, 4)
acti_label = 'Activation Function '+'$\Delta$'
...
def raioPlotter(cdicts, est_dict, beats_ann, title = None,
                ...
                acti_label = acti_label, y_tick_dict = y_tick_dict, 
                figsize = figsize):
    ### modify the legend
    # simp_legend = {k: '$B_\mathrm{'+k+'}$' for k in cdicts.keys()}
    simp_legend = {k: k for k in cdicts.keys()}
    ...
    fig, ax = plt.subplots(len(cdicts)+1, 1, figsize = figsize, 
    ...
    for ind, (post_proc, c_arr) in enumerate(cdicts.items(), 1):
        ...
        ax[ind].set_ylabel('Metrical Level ', fontsize = fontsize)
        im.set_clim(-0, 1)
    ax[ind].set_xlabel('Time (frame, FPS = 100)', fontsize = fontsize)
    ax[ind].get_yaxis().set_label_coords(-0.14,0.5)
    ### legend
