...
def train(model, device, train_loader, optimizer, bce_weight):
    ...
    for x, yb in pbar:
        ...
        optimizer.zero_grad()
        out = model(x)
        beat_loss = nn.BCEWithLogitsLoss(pos_weight = torch.tensor([bce_weight]).to(device))( out['x_act'].reshape((-1,)), yb.reshape((-1,)))
        loss = beat_loss
        ...
        act = out['x_act'].detach().cpu().numpy()[0, :]
...
def main():
    ...
    exp_setting = {
        ...
        'bce_weight' : 3,
        'batch_size' : 8,
        'gsmn':  True, # Gaussian smoothing and maz-normalizaing the training target
        }
    model_setting = {
        'N_bands':8, 
        'gamma_init':10, 
