...
def train(model, device, train_loader, optimizer, bce_weight):
    ...
    for x, yb in pbar:
        ...
        plpn10_loss = nn.BCEWithLogitsLoss(pos_weight = torch.tensor([bce_weight]).to(device))(out['N10'][:, :out['fused_nov'].shape[1]].reshape((-1,)), yb.reshape((-1,)))
        plpn5_loss = nn.BCEWithLogitsLoss(pos_weight = torch.tensor([bce_weight]).to(device))(out['N5'][:, :out['fused_nov'].shape[1]].reshape((-1,)), yb.reshape((-1,)))
        plpn3_loss = nn.BCEWithLogitsLoss(pos_weight = torch.tensor([bce_weight]).to(device))(out['N3'][:, :out['fused_nov'].shape[1]].reshape((-1,)), yb.reshape((-1,)))
        ...
        train_loss['plpn3'] += plpn3_loss.detach().item()
        train_loss['plpn5'] += plpn5_loss.detach().item()
        train_loss['plpn10'] += plpn10_loss.detach().item()
        ...
        if not loss.isnan():
            ...
            torch.nn.utils.clip_grad_norm_(model.parameters(), 3, error_if_nonfinite=False)
...
def valid(model, device, valid_loader, bce_weight ):
    ...
    with torch.no_grad():
        for x, yb in pbar:
            ...
            valid_loss['plpn3'] += plpn3_loss.detach().item()
            valid_loss['plpn5'] += plpn5_loss.detach().item()
            valid_loss['plpn10'] += plpn10_loss.detach().item()
...
def main():
    ...
    exp_setting = {
        ...
        'fs_nov': Fs_nov,
        'temp': 1, # temperature
        'theta':'start_{}_stop_{}_num_{}_log'.format(start, stop, num),
        'loss-type': 'BCE',
        'target-gsmn': True,
        'bce_weight' : 3,
        'batch_size' : 8,
    ...
    model_setting = {
        ...
        'diff_trainable' :  False,  
        'loc_avg_trainable': False,
        'weighted_sum_trainable':  False, 
    ...
    if continue_from_old:
        ...
        print('load pretreind SFX model...')
        model_path = os.path.join('./experiments/SFX', 'model.chkpnt')
        model.NovNet.load_state_dict(torch.load(model_path, map_location=device, weights_only=True)['state_dict'])
        # summary( model)
    ...
    for epoch in t:
        ...
        train_plpn3_losses.append(train_loss['plpn3']/train_loss['len'])
        train_plpn5_losses.append(train_loss['plpn5']/train_loss['len'])
        train_plpn10_losses.append(train_loss['plpn10']/train_loss['len'])
        ...
        valid_plpn3_losses.append(valid_loss['plpn3']/valid_loss['len'])
        valid_plpn5_losses.append(valid_loss['plpn5']/valid_loss['len'])
        valid_plpn10_losses.append(valid_loss['plpn10']/valid_loss['len'])
